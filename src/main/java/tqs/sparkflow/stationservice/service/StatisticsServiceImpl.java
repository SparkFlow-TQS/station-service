...
public class StatisticsServiceImpl implements StatisticsService {
    ...
    public StatisticsDTO.CurrentMonthStats getCurrentMonthStatistics(Long userId) {
        ...
        List<ChargingSession> sessions = chargingSessionRepository.findFinishedSessionsByUserInPeriod(
            userId.toString(), startOfMonth, endOfMonth);
        ...
        for (ChargingSession session : sessions) {
            if (session.getStartTime() != null && session.getEndTime() != null) {
                double duration = ChronoUnit.MINUTES.between(session.getStartTime(), session.getEndTime()) / 60.0;
                int kwh = (int) Math.round(duration * DEFAULT_KWH_PER_HOUR);
                totalKwh += kwh;
                
                // Get station price
                Optional<Station> station = stationRepository.findById(Long.parseLong(session.getStationId()));
        ...
        stats.setAvgCostPerSession(sessions.size() > 0 ? Math.round((totalCost / sessions.size()) * 100.0) / 100.0 : 0.0);
    ...
    public List<StatisticsDTO.MonthlyData> getMonthlyData(Long userId, int months) {
        ...
        for (int i = 0; i < months; i++) {
            ...
            List<Booking> bookings = bookingRepository.findBookingsByUserInPeriod(
                userId.toString(), startOfMonth, endOfMonth);
            ...
            data.setReservations(bookings.stream()
    ...
    public List<StatisticsDTO.WeeklyData> getWeeklyDataCurrentMonth(Long userId) {
        ...
        Map<Integer, List<ChargingSession>> sessionsByWeek = sessions.stream()
            ...
            .collect(Collectors.groupingBy(session -> 
                session.getStartTime().get(weekFields.weekOfYear())));
        ...
        for (Map.Entry<Integer, List<ChargingSession>> entry : sessionsByWeek.entrySet()) {
            ...
            if (!weekSessions.isEmpty()) {
                LocalDateTime firstSession = weekSessions.stream()
                    .map(ChargingSession::getStartTime)
                    .min(LocalDateTime::compareTo)
                    .orElse(null);
                LocalDateTime lastSession = weekSessions.stream()
                    .map(ChargingSession::getStartTime)
            ...
            data.setReservations(new ArrayList<>()); // No reservations for weekly data in this context
            weeklyData.add(data);
        }
    ...
    public StatisticsDTO.PeriodDetails getPeriodDetails(Long userId, String type, String value) {
        ...
        if ("month".equals(type)) {
            YearMonth yearMonth = YearMonth.parse(value + "-01", DateTimeFormatter.ofPattern("yyyy-MM-dd"));
        ...
        details.setAvgCostPerSession(sessions.size() > 0 ? Math.round((totalCost / sessions.size()) * 100.0) / 100.0 : 0.0);
        details.setReservations(bookings.stream()
            .map(this::convertToBookingDTO)
            .collect(Collectors.toList()));
    ...
    private BookingDTO convertToBookingDTO(Booking booking) {
        ...
        if (booking.getStartTime() != null && booking.getEndTime() != null) {
            double duration = ChronoUnit.MINUTES.between(booking.getStartTime(), booking.getEndTime()) / 60.0;
            int kwh = (int) Math.round(duration * DEFAULT_KWH_PER_HOUR);
