...
public interface StationRepository extends JpaRepository<Station, Long> {
    ...
    @Query("SELECT s FROM Station s WHERE " + "(:minPower IS NULL OR s.power >= :minPower) AND "
    ...
    List<Station> findStationsByFilters(@Param("minPower") Integer minPower,
            @Param("maxPower") Integer maxPower, @Param("isOperational") Boolean isOperational,
            @Param("status") String status, @Param("city") String city,
            @Param("country") String country, @Param("minPrice") Double minPrice,
    ...
    @Query(value = "SELECT * FROM stations s WHERE "
            ...
            + "ORDER BY s.id LIMIT 500", nativeQuery = true)
    List<Station> findStationsByFiltersWithLocation(@Param("minPower") Integer minPower,
            @Param("maxPower") Integer maxPower, @Param("isOperational") Boolean isOperational,
            @Param("status") String status, @Param("city") String city,
            @Param("country") String country, @Param("minPrice") Double minPrice,
            @Param("maxPrice") Double maxPrice, @Param("latitude") Double latitude,
            @Param("longitude") Double longitude, @Param("radius") Integer radius);
}
